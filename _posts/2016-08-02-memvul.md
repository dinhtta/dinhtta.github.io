...
is driven largely by a perpetual arm race between attacks and defenses, between breaking stuff and patching stuff. Helped
...
exploits. Vulnerabilities are the way in, allowing attackers to hijack the programs' control flows. To search for 
vulnerabilities, attackers often look for places where memory can be corrupted, which could be as simple as going
through the source code. Crafting and launching the corresponding exploits often require more craftsmanship. Defenders
use this division to gear their strategies, either to nip attacks at their buds by ensuring memory safety, or by making
exploits more difficult if not impossible.  
