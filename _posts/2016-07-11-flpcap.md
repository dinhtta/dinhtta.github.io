...
&nbsp; Solutions|&nbsp; Synchronous network &nbsp;|&nbsp; No solution exists
|---|---|---

### Problem scopes
**Distributed consensus vs. Replicated storage.** Distributed consensus concerns with getting a number of parties to
agree on a value. One common manifestation of consensus is _leader election_, wherein the nodes decide who should e
coordinating the group. An equally important form of consensus is at the heart of replicated state machine, wherein
every nodes agree on the sequence of events applied to a state machine. In databases, distributed commit is also a
consensus problem: nodes agree on whether to commit the operation or not. 

Replicated storage is essentially a form of replicated state machine, where state is a memory location and events are
read and write operation. Although reaching agreement is important, concern of availability is equally essentially and
sometime taking priority over agreement. In leader election or commitment, disagreement is not acceptable (all or
nothing); but for replicated storage, refusing a read or write request is sometimes worse than having diverge replicas. 

One simplified way to think of these two problems is that distributed consensus is often used when the task or
data is partitioned/sharded to multiple nodes, whereas replicated storage is more suitable when the task or data is in
multiple copies. Think 2PC in the first case, and primary-backup in the second.  

**Node failure vs. Network Partition.**
FLP considers node fail by crashing: it simply stops working. CAP considers network failure which leads to partitions.

<div class="highlight" style="font-size: 20px">
In an asynchronous model, these two modes of failure are indistinguishable, since no response and very long delay are
essentially the same.  But in a synchronous model, network partition is a stronger failure model (more difficult to deal
with) than crash failure. 
</div>

In synchronous settings, when a node is detected, it can be marked dead and removed from the system execution. However,
when partition is detected, nodes in other partition cannot be marked as dead. Instead, both partitions must continue to
work, but in isolation and knowing that their states can diverge.  

### Formalization
Written in 1985, FLP was formulated and shown rigorously in I/O Automata model. Both safety and liveness were stated
formally. In contrast, CAP was initially presented as a conjecture and later formalized by Gilbert and Lynch (G&L) [2].
It is understandable, then, that there is a gap between what Brewer meant (or thought to have meant) and what was
captured by G&L formal model. The main discrepancy lies in the definition of availability. Brewer's conjecture implies
an externally observed characteristic of the system, i.e. user's perspective based on uptime and delay. In G&L's
definition, availability is the system's internal property, i.e. liveness. A G&L's available system may take unbounded
time to response, but it is unacceptable and useless in the pragmatic sense of availability. I'll come back to this
points later when discussing how CAP has evolved.   

### Impact
Both FLP and CAP help one to rule out infeasible design at early stage, as well as to recognize over-claims and
hidden flaws in systems which go against the results.  

FLP was particularly interesting, as it identifies a fundamental problem with asynchronous models: only a
single potential failure will completely annihilate consensus guarantees. As having consensus is at the heart
of distributed systems, the result is a damaging one. The flip side is that it forces one to be _explicit_
about the trade-offs: ensuring either safety or liveness, and optimizing another. More importantly, it forces
one to re-examine the assumption of network being asynchronous: to which extend relaxing asynchrony affect the
trade-offs. When the network is synchronous, FLP does not apply, i.e. it is possible to have all 3 properties.

CAP was not entirely ground breaking at the time it came out, but it was quickly picked up by system
designers. G&L's proof put CAP in more formal ground. But misunderstanding of the original conjecture and
misinterpretation of the proof started to appear that Brewer wrote a paper to clarify his position [3], and there
is paper as later as December 2015 proposing extension to CAP. 

<div class="highlight" style="font-size: 20px">
While FLP is more of a strict rule in designing systems, CAP is more useful as 3 dimensions in a design space
...
partition-tolerance all at the same time.    

---

# Living With FLP
Despite its negativity, FLP does not mean we cannot build anything useful. First, FLP only shows that it is
not possible to _guarantee_ consensus under failure in asynchronous network. It means that consensus can be
achieved _some time_. Particularly, a system which trades safety for liveness or vice versa is far from
useless. 
...
algorithm may never terminates.  

* **Live but not safe**: 3-phase commit (3PC) trades safety for liveness: the algorithm always terminates, but
there exists unsafe runs. More recently, Blockchain (exemplified by Bitcoin) also implements a distributed
...
* **Safe and live**: Technically one can modify 2PC by removing all fault-tolerance stuff, thereby making
it both safe and live. Though using this modified algorithm is very risky, since it's difficult to ensure that
there will never be any failure. If you chance reader happens to know of a better example (or that it is not
possible to have), please send me a note.   

### Overcoming FLP
Both 2PC and Paxos are widely used to build or support many classes of distributed systems. FLP
impossibility does not stop the community from creating robust consensus algorithms. Interesting, many see FLP
with optimistic eyes: yes, FLP marks a large portion of the distributed system design space from the realm of
possibility, yet the remaining space is vast and inviting. 

* **Failure detector:** perhaps the most significant study after FLP is by Chandra and Toueg, which asks the
question of how one can overcome FLP in asynchronous systems [4]. Particularly, CT considered how to _augment_ the
asynchronous system to make consensus possible. They captured such augmentation via a distributed failure
detector: each node can query its local failure detector whether another node has failed or not. By doing
this, CT turns consensus into an application which uses the failure detector. CT results, amid heavy
formalization, are very neat:
   * Consensus and atomic broadcast --- the mechanism behind ISIS and Zab/Zookeeper --- are equivalent.
   * Consensus is possible in asynchronous _if and only if_ it has access to a failure detector $$\Omega$$. CT
   showed how to implement consensus given a failure detector. 
   * $$\Omega$$ can be very weak: it may make infinite number of mistakes, but _eventually_ it detects
   correctly one failure and does not wrongly identify a non-failing node as failed. 

Beautiful as they are, failure detectors are only a theoretical construct: it says that solving consensus in
asynchronous networks is the same as building a weak failure detector $$\Omega$$. CT emphasized many times in
their paper that they do not concern with implementation of such detector. Of course it follows from FLP that
$$\Omega$$ cannot be built in asynchronous networks. Nevertheless, it is possible in other settings. 

* Synchronous network: one can build a perfect failure detector in this setting based on time-out. Thus, it is
possible to have safety, liveness and fault-tolerance at the same time. Take Paxos as an example, we can
modify the algorithm to simply mark a node as fail when it does not response within the time limit. The
remaining nodes can then happily restart and proceed with the original algorithm. 

* Partially synchronous network: Dwork and Lynch explored a model in between wholly asynchronous and wholly
synchronous: the network is sometimes synchronous, sometimes asynchronous, but over time it stabilizes to
synchronous [5]. They showed that partial synchrony can be parameterized and in some cases it is possible to solve
consensus. This study predated that of failure detector, and CT later demonstrated how a failure detector can
be built in partially synchronous network. Not only does the failure detector agree with partial synchrony, it
is more general, simple and intuitive.    

---

# Evolving CAP
The original CAP conjecture was intuitive for people who build distributed systems. When replicas are isolated
by partitions, one must either return potentially inconsistent values or refuse requests. This original
version of CAP has since undergone various refinement, clarification and extension that makes it better
captures subtle trade-offs in existing systems.  
