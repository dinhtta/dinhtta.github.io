...
The surprising result here is that only one potential crash failure is needed to the impossibility to take
...
* Synchronous network: FLP does not apply. When network delay is bounded, it is possible to have safety,
liveness and fault-tolerance at the same time. Take Paxos as an example, we can modify the algorithm to simply
mark a node as fail when it does not response within the time limit. The remaining nodes can then happily
restart and proceed with the original algorithm. 

* Partially synchronous network: wholly synchronous networks are not entirely realistic, thus some researchers
explore settings wherein the network is sometimes synchronous, sometimes asynchronous, but over time it
stabilizes to synchronous. [4] shows that if eventual synchrony can be guarantee, we can solve consensus.
Yay!. Furthermore, solutions exists with up to $$n/2$$ crash failure.  

* Failure detector: 

## Evolving CAP
Examples of CA, CP and AP systems.  
...
[1] Quora thread on FLP and CAP equivalent. 

[2] Gilbert and Lynch's proof. 

[3] CAP 12 years later. 
