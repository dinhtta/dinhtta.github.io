...
* A miner is an interactive Turing machine (ITM) with an input tape. It executes a protocol $$\Pi$$, e.g. the Bitcoin backbone.  
...
An environment $$Z$$ supplies input and issues queries to the miner. Its *view* $$V$$ is a random variable
where the randomness comes from $$Z$$'s coins and the miners'. $$V$$ represents the visible states of the
entire network. Security of the protocol $$\Pi$$ is specified as a predicate over $$V$$. In particular,
$$\Pi$$ satisfies $$P$$ if $$P(V)$$ is false with negligible probability.  

### The protocol
The *backbone* of Bitcoin consists of 
### Security property
