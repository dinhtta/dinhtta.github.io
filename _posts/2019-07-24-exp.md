...
* Load: the number of requests received at $$S$$. This should be varied. Note that I consider *load* as what the client
generates, which may differ to what $$S$$ experiences. The latter is called **offered load**. Sometimes, the client
may think it generates a high load, but what $$S$$ receives is actually very small. 
...
## Load Generator
We want to generate load such that it approximates how $$S$$ would be used in practice. Therefore, a load generator
typically falls into one of the two categories:

* **Closed-loop:** there are $$N$$ clients, each sends a request, blocks until it receives a response, then thinks for time
$$T$$, then sends again. This model resembles what happens at (deep) backend servers, like databases or key-value
...
where $$L$$ is the **mean** number of requests in the system. $$L$$ includes requests being queued, and ones that being
serviced. For example, suppose we know that a disk can service $$\lambda = 50$$ requests per second, and we observe that
the average length queue at the disk is $$10$$, then the average time a request spent in the system is $$10/50 = 0.2$$
second.  

**Important assumptions of queue theory**: The system is *stable*, in the sense that $$\lambda \leq \mu$$. Intuitively,
it is stable only when the arrival rate is less than what the system can handle. Otherwise, when $$\lambda > \mu$$, the
queue goes to infinity, and the mean values like $$R$$ do not exist.  

**Utilization rule**: when there is no queue, the number of requests in the system is either $$0$$ (when the system
is not busy), or $$1$$ when it is busy. Utilization, being the average time when the system is busy, is $$L$$. In other
words, $$\rho = L = \lambda.\mu$$. $$\rho$$ can be interpreted as offered load, and $$\rho \in [0,1]$$. We do not
consider $$\rho > 1$$ (or $$\lambda > \frac{1}{\mu}$$) because the system becomes unstable. Although $$\rho = 1$$ is
a possibility, it is rarely a good idea to push the system to this state, which Gil Tene agrees [4]. Remember that
$$\lambda$$ and $$\mu$$ are **means**, so there are certainly some values that pushing the system over the edge. 

> The key point is that there is no point of looking at the system behavior when it saturates, you only want to push that
> saturation point further, or to make sure that performance before saturation is good enough.  
...
**Increasing $$N$$ in closed-loop model gets it closer to the open-loop.**   
