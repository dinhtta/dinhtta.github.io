...
People have been analyzing these two models with queue theory. The key result is **Little's Law**, that relates input of
a system and its capacity with the queue length. In particular, let $$\sigma$$ be the expected service time (not to be
confused with response time [4]):

* For closed-loop system with $$T=0$$, the 
